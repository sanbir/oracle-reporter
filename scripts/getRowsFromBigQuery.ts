    ...
    try {
        ...
        const query = `
            with chunk as (select epoch,
               ...
               , withdrawals_epochs as (
            SELECT epoch, val_id, ROW_NUMBER() over (partition by val_id order by epoch) as withdrawal_number
            FROM chunk
            where val_balance_withdrawn is not null
                )
                , withdrawal_periods as (
            select chunk.epoch, chunk.val_id, chunk.epoch_date, val_balance_withdrawn, COALESCE (att_earned_reward, 0) +
                COALESCE (propose_earned_reward, 0) +
                COALESCE (sync_earned_reward, 0) -
                COALESCE (att_penalty, 0) -
                COALESCE (propose_penalty, 0) -
                COALESCE (sync_penalty, 0) as earned_rewards, we.withdrawal_number, COALESCE (
                we.withdrawal_number, LAST_VALUE(we.withdrawal_number ignore nulls)
                OVER (partition by chunk.val_id ORDER BY chunk.epoch rows between unbounded preceding and current row) +
                1, 1
                ) AS withdrawal_period
            FROM chunk
                left join withdrawals_epochs we
            on we.epoch = chunk.epoch and we.val_id = chunk.val_id)
            ...
            from (select val_id,
                         ...
                         case
                             when
                                 sum(mod(val_balance_withdrawn, cast(32 * power(10, 9) as int))) - sum(earned_rewards) <
                                 sum(earned_rewards)
        ...
        await sleep(5000)
        logger.info('Re-trying', valIds, startDate, endDate)
        return await getRowsFromBigQuery(valIds, startDate, endDate)
    }
}
